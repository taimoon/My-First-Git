#include<stdio.h>
#include"src/C_Forward_List_Macro.h"
...
ForwardListConstruct(int,printInt);
ForwardListConstruct(float,printFloat);
void printTest(node(int, ForwardList) *list){
    printSeq(int, ForwardList, list);printf("\n");
    //printNodeBack(list); //toggle this for double linked list and circular double linked list
    printf("\n");
}
void printTest2(node(float, ForwardList) *list){
    printSeq(float, ForwardList, list);printf("\n");
    //printNodeBack(list); //toggle this for double linked list and circular double linked list
    printf("\n");
}
int main()
{
    int testSize = 10;
    node(int, ForwardList) *list = NULL;
    node(float, ForwardList) *list2 = NULL;
    printf("Test insert data at the start\n");
    for(int i = 0; i < testSize; ++i){
        pushFront(int, ForwardList, list, i);
        pushFront(float, ForwardList, list2, i);
    }
    ...
    for(int i = 0; i < testSize; ++i){
        printf("%d ", getNode(int, ForwardList, list, i)->val);
        printf("%.2f ", getNode(float, ForwardList, list2, i)->val);
    }

    printf("\n");

    printf("\nIs getLen() working correctly?: %d", getLen(int, ForwardList, list)==testSize);
